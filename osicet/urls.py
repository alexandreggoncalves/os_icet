...
# Mapeia a aplicação SPA e os endpoints JSON consumidos pelo frontend.
urlpatterns = [
    # Entrada principal do React servido pelo Django.
    path("", views.index, name="index"),
    path("index.html", views.index, name="index_html"),
    # Bootstraps separam dados públicos dos dados dependentes de autenticação.
    path("api/public/bootstrap", views.public_bootstrap),
    path("api/admin/bootstrap", views.admin_bootstrap),
    # Endpoints de autenticação, primeiro acesso e recuperação de senha.
    ...
    # Fluxo de solicitação, status, histórico e anexos.
    ...
    # Cadastros administrativos usados pelas telas de gerenciamento.
    ...
    # Arquivos estáticos versionados do bundle local.
    re_path(r"^(?P<path>(assets|vendor)/.*)$", serve, {"document_root": settings.BASE_DIR / "static"}),
]

if settings.DEBUG:
    # Em desenvolvimento, os anexos são servidos pelo próprio Django.
